...
//elimina todos los espacios y tabuladores antes del primer caracter 
//"valido" de una cadena de caracteres
...
int main() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        tamTotalMinimo += MAXLINE * sizeof(char);
        tamTotalMinimo += sizeof(char);
        tamTotalMinimo += 3 * sizeof(int);
    }
                
    printf("Tamaño total ocupado en memoria: %d bytes\n", tamTotalReal);
    printf("tamaño que realmente necesita: %d bytes\n", tamTotalMinimo);
